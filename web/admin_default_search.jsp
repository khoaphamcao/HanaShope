<%-- 
    Document   : newjsp
    Created on : Jan 21, 2021, 4:11:07 PM
    Author     : Khoa Pham
--%>
...
<html>
    ...
    <head>
        ...
        <style> <%@include file="css/admin_search.css" %> </style>
    </head>
    <body>
        ...
        <c:set var="result" value="${sessionScope.ADMIN_RESULT}"/>
                ...
                <c:forEach var="dto" items="${result}" varStatus="counter">
                    ...
                    <form action="DispatchServlet" method="POST">
                        <tr>
                            ...
                            <td> ${dto.description}
                                <input type="hidden" name="description" value="${dto.description}" /> </td>
                            <td> ${dto.price} <input type="hidden" name="price" value="${dto.price}" /> </td>
                            <td><fmt:formatDate type = "date"  value = "${dto.createDate}" /></td>
                            <td> ${dto.category} <input type="hidden" name="category" value="${dto.category}" /> </td>
                            <td> ${dto.quantity} 

                                <input type="hidden" name="quantity" value="${dto.quantity}" /> </td>
                            <td> <c:if test="${dto.status}">
                                    Active
                                </c:if> 
                            ...
                            <td> 
                                <button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onclick="return confirm('Do you want to Delete this product?')" name="btAction" value="Delete">
                                    Delete
                                </button>
        ...
        <div class="paging">
            <c:forEach begin="1" end="${requestScope.END_PAGE}" var="i" >
                <a href="DefaultAdminSearchServlet?index=${i}&txtSearch=${requestScope.SEARCH_VALUE}&searchOption=${requestScope.OPTION}&txtPriceFrom=${requestScope.PRICE_FROM}&txtPriceTo=${requestScope.PRICE_TO}">${i}</a>
            </c:forEach>
        </div>
    </c:if>
